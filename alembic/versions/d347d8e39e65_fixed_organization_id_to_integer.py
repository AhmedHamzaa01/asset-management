"""Clean schema - integer asset ids, uuid user/org ids

Revision ID: d347d8e39e65
Revises: 
Create Date: 2026-06-25 05:25:39.649104
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint(
            'source_asset_id', 'target_asset_id', 'relationship_type', 'organization_id',
            name='uq_asset_relationship'
        ),
    )
    op.create_index('ix_asset_relationships_id', 'asset_relationships', ['id'], unique=False)
    op.create_index('ix_asset_relationships_organization_id', 'asset_relationships', ['organization_id'], unique=False)
    op.create_index('ix_asset_relationships_source_asset_id', 'asset_relationships', ['source_asset_id'], unique=False)
    op.create_index('ix_asset_relationships_target_asset_id', 'asset_relationships', ['target_asset_id'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index('ix_asset_relationships_organization_id', table_name='asset_relationships')
    op.drop_index('ix_asset_relationships_id', table_name='asset_relationships')
